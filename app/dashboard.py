...
with st.sidebar:
    ...
    # GDPR consent notice
    st.caption(
        "⚖️ **Data Processing Notice:** Uploaded data is processed by OpenAI GPT-4o "
        "for AI analysis. Employee IDs and PII are stripped before any data leaves "
        "this server. Data is held in memory only and auto-deleted after 24 hours. "
        "See our data minimization practices for details."
    )
    consent = st.checkbox("I confirm I have authorization to upload this data", key="_gdpr_consent")
...
if analyze_button:
    if not st.session_state.get("_gdpr_consent"):
        st.sidebar.error("Please confirm data authorization before uploading")
