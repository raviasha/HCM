...
def analyze_schema(company_name: str) -> dict:
    """Request PII-classified schema review for uploaded data."""
    with httpx.Client(timeout=TIMEOUT) as client:
        resp = client.post(
            _url("/api/data/analyze-schema"),
            data={"company_name": company_name},
        )
        resp.raise_for_status()
        return resp.json()
...
def approve_schema(company_name: str, columns: list[dict], feedback_columns: list[dict] = None) -> dict:
    """Submit user-approved PII classifications."""
    payload = {"company_name": company_name, "columns": columns}
    if feedback_columns:
        payload["feedback_columns"] = feedback_columns
    with httpx.Client(timeout=TIMEOUT) as client:
        resp = client.post(
            _url("/api/data/approve-schema"),
            json=payload,
        )
        resp.raise_for_status()
        return resp.json()
