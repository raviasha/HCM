...
def run_ml_pipeline(
    df: pd.DataFrame,
    mapping: dict,
    pii_classifications: list | None = None,
) -> dict:
    """
    ...
    Args:
        pii_classifications: Optional list of ColumnClassification objects.
            Columns with handling 'exclude' or 'aggregate_only' are removed
    ...
    try:
        X, y, feature_names, cat_cols, num_cols, outliers_capped = (
            _prepare_features(df, mapping, pii_classifications)
        )
...
def _prepare_features(
    ...
    Columns classified as 'exclude' or 'aggregate_only' by the PII
    classifier are removed from the feature set.
    ...
    # Exclude PII columns (identifiers, names, emails, etc.)
    if pii_classifications:
        _PII_EXCLUDE = {"exclude", "hash", "aggregate_only"}
        for cc in pii_classifications:
            if cc.handling in _PII_EXCLUDE:
                exclude.add(cc.column_name)
        logger.info("PII-excluded columns from ML features: %s",
                    {cc.column_name for cc in pii_classifications
                     if cc.handling in _PII_EXCLUDE})
...
def _risk_scoring(
    ...
    # Determine if the id column should be hashed (PII handling)
    _hash_id = False
    if pii_classifications:
        _pii_map = {c.column_name: c.handling for c in pii_classifications}
        if _pii_map.get(id_col) in ("exclude", "hash"):
            _hash_id = True
        # Also: if mapping didn't identify an id col, try PII identifiers
        if not mapping.get("employee_id_col"):
            for c in pii_classifications:
                if c.pii_category == "identifier" and c.column_name in df.columns:
                    id_col = c.column_name
                    _hash_id = True
                    break
    ...
    target_col = mapping["target_col"]
    target_type_local = mapping.get("target_type", target_type)
    if "_target_flag" in df.columns:
        valid_mask = df["_target_flag"].notna().values
    elif target_type_local == "binary":
    ...
    for _, row in top_risk.iterrows():
        ...
        if _hash_id and raw_id:
            display_id = hashlib.sha256(raw_id.encode()).hexdigest()[:8]
        else:
            display_id = raw_id
        emp = {
    ...
    if dept_col in valid_df.columns:
        dept_risk = valid_df.groupby(dept_col).agg(
            ...
            high_count=("_risk_level", lambda x: int((x == "High").sum())),
            total=("_risk_score", "count"),
        ).reset_index()
        dept_risk["high_count"] = dept_risk["high_count"].astype(int)
        dept_risk["total"] = dept_risk["total"].astype(int)
